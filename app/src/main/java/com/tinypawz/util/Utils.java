...
public class Utils {
	...
	public static final String ATTRIBUTE_SCHEMA = "";
...
//	}
	...
	public static int getSquareImageDimen(Context context) {
		DisplayMetrics displayMetrics = new DisplayMetrics();
		WindowManager windowmanager = (WindowManager) context.getApplicationContext().getSystemService(Context.WINDOW_SERVICE);
		windowmanager.getDefaultDisplay().getMetrics(displayMetrics);

		// width in dp -- 5 dp on each side
		int width = Math.round(displayMetrics.widthPixels / displayMetrics.density);
		int pxWidth = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (width - 5*2), displayMetrics);

		return pxWidth;
	}

	public static int getSquareImageDimenWithoutMargin(Context context) {
		DisplayMetrics displayMetrics = new DisplayMetrics();
		WindowManager windowmanager = (WindowManager) context.getApplicationContext().getSystemService(Context.WINDOW_SERVICE);
		windowmanager.getDefaultDisplay().getMetrics(displayMetrics);
	...
	public static int getFullWidth(Context context) {
		...
		int var = (int) (width* Resources.getSystem().getDisplayMetrics().density);
		int height = displayMetrics.heightPixels;

		System.out.println("screen var" + var );
